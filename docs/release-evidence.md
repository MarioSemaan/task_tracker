...
- **Latest run**: GitHub Actions ran the full suite on push to `final-project`. The workflow installed dependencies from `requirements.txt` (which includes `pytest` and `httpx`) and ran `python -m pytest -v`. All 20 tests passed. Run is visible at: `https://github.com/<your-username>/task-tracker/actions` under the most recent push to `final-project`.
